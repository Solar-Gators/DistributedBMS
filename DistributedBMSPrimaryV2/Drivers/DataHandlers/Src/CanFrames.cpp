...
bool decodeVoltageExtremes(const uint8_t* data, uint16_t& highV, uint16_t& lowV, uint8_t& lowIdx,
    ...
    if (data[0] != VOLTAGE_EXTREMES) {
        return false;
    }
    highV = static_cast<uint16_t>((static_cast<uint16_t>(data[2]) << 8) | data[1]);
    lowV = static_cast<uint16_t>((static_cast<uint16_t>(data[4]) << 8) | data[3]);
    lowIdx = data[5];
    highIdx = data[6];
    faults = data[7];
    return true;
...
bool decodeAverages(const uint8_t* data, float& avgTemp, uint16_t& avgVoltage, uint8_t& numCells) {
    ...
    avgVoltage = static_cast<uint16_t>((static_cast<uint16_t>(data[6]) << 8) | data[5]);
    numCells = data[7];
    return true;
}
